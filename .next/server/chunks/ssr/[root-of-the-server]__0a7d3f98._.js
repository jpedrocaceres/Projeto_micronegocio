...
const NewAppointmentPage = ()=>{
    ...
    const fetchServices = async ()=>{
        ...
        try {
            ...
            const { data, error } = await supabase.from('services').select('*').eq('user_id', user.id).eq('is_active', true).order('name');
            if (error) {
    ...
    const generateAvailableSlots = async ()=>{
        ...
        const { data: existingAppointments } = await supabase.from('appointments').select('start_time, end_time').eq('user_id', user.id).eq('appointment_date', selectedDate).in('status', [
            'scheduled',
            'confirmed'
        ]);
        const slots = [];
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0a7d3f98._.js.map